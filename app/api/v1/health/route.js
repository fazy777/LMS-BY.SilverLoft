...
export async function GET() {
  ...
  try {
    ...
    if (auth) {
      diagnostics.firebaseAdmin.status = (diagnostics.firebaseAdmin.hasClientEmail && diagnostics.firebaseAdmin.hasPrivateKey) || diagnostics.firebaseAdmin.hasServiceAccount
        ? 'configured'
        : 'missing_credentials';
    }
  } catch (err) {
  ...
  const allHealthy = diagnostics.database.status === 'connected' && diagnostics.firebaseAdmin.status === 'configured';
