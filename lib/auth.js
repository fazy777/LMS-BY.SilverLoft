...
function getFirebaseAdminCredentials() {
  ...
  const rawPrivateKey = (process.env.FIREBASE_PRIVATE_KEY || process.env.FIREBASE_ADMIN_PRIVATE_KEY)?.trim();
  const projectId = (process.env.FIREBASE_PROJECT_ID || process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || 'lms-nextjs-42ab6')?.trim();
  ...
  if (!firebaseAuth) {
    try {
      ...
      if (apps.length === 0) {
        ...
        const projectId = credentials?.projectId || credentials?.project_id || process.env.FIREBASE_PROJECT_ID || process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || 'lms-nextjs-42ab6';
        ...
        if (credentials) {
          try {
            ...
            lastFirebaseError = `cert error: ${certErr.message}`;
            console.error('[Firebase Admin] Failed to parse cert credentials:', certErr.message);
          }
        } else {
          lastFirebaseError = 'No Firebase Admin credentials found in environment variables';
        ...
        try {
          ...
          lastFirebaseError = `initializeApp error: ${appErr.message}`;
          console.error('[Firebase Admin] initializeApp error:', appErr.message);
      ...
      lastFirebaseError = `dynamic import error: ${importErr.message}`;
      console.error('[Firebase Admin] Failed to dynamically load firebase-admin:', importErr.message);
    }
  }
  return firebaseAuth;
}
...
export async function withAuth(req) {
  ...
  if (!auth) {
    throw new AppError('UNAUTHENTICATED', 'Firebase Admin auth service is unavailable.', 401);
  }
  ...
  try {
    ...
    decoded = await auth.verifySessionCookie(cookie.value, false);
  } catch {
...
export async function getServerUser() {
  try {
    ...
    const decoded = await auth.verifySessionCookie(cookie.value, false);
    const [rows] = await query(
      `SELECT id, firebase_uid, email, display_name, avatar_url,
              is_instructor, is_admin, status
         FROM users
