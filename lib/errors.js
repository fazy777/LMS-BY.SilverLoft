...
export function handle(fn) {
  return async (req, ctx) => {
    try {
      ...
      return fail(500, 'INTERNAL_ERROR', err?.message || 'Something went wrong on our side.', {
        name: err?.name,
        message: err?.message,
...
/** Safely extract searchParams from request, handling relative or absolute URLs */
export function getSearchParams(req) {
  if (req?.nextUrl?.searchParams) return req.nextUrl.searchParams;
  try {
    return new URL(req.url, 'http://localhost:3000').searchParams;
  } catch {
    return new URLSearchParams();
  }
}

/** Safely extract origin/baseUrl from request */
export function getBaseUrl(req) {
  ...
  try {
    return new URL(req.url, 'http://localhost:3000').origin.replace(/\/+$/, '');
  } catch {
