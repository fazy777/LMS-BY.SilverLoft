...
function getSslConfig(isLocalhost) {
  if (process.env.MYSQL_SSL === 'false') {
    return undefined;
  }
  // TiDB Cloud, Aiven, Railway, AWS RDS require TLS 1.2+
  if (
    ...
    return {
      minVersion: 'TLSv1.2',
      rejectUnauthorized: process.env.MYSQL_SSL_REJECT_UNAUTHORIZED === 'true' || true,
    };
  }
  return undefined;
}
...
function createDbPool() {
  ...
  if (connectionUri) {
    const isLocalUri = connectionUri.includes('localhost') || connectionUri.includes('127.0.0.1') || connectionUri.includes('::1');
    const ssl = getSslConfig(isLocalUri);
  ...
  const ssl = getSslConfig(isLocalhost);
